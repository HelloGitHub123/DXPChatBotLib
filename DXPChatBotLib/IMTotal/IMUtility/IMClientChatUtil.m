...
@implementation IMClientChatUtil
...
+ (NSString *)getNetSignatureWithContent:(id)content andDate:(NSString *)gmtDate {
    NSString *normalSignature = [NSString stringWithFormat:@"%@\n%@\n%@",
                                 ...
                                 gmtDate];
    //此处用原生自带的base64
    NSData *data = [[[self hmacSha1:normalSignature hmacKey:[IMConfigSingleton sharedInstance].secretKey] hexLower] dataUsingEncoding:NSUTF8StringEncoding];
...
+ (UCCSecurityResult *)hmacSha1WithData:(NSData *)hashData hmacKey:(NSString *)key
{
    ...
    UCCSecurityResult *result = [[UCCSecurityResult alloc] initWithBytes:digest length:CC_SHA1_DIGEST_LENGTH];
    free(digest);
    cKey = nil;
...
@implementation UCCSecurityResult
...
- (NSString *)hex
{
    UCCSecurityEncoder *encoder = [UCCSecurityEncoder new];
...
@implementation UCCSecurityEncoder
...
@implementation UCCSecurityDecoder
